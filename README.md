...
- Replaces static history tracking with **Path-Integrated Teleological Drift**, computing a `teleological_integral` by comparing the `observed_drift` of an ecosystem against its `expected_drift` from its `origin_epoch`, preserving replayability without unbounded state cloning.
...
- Enforces **Basin-Local Thermodynamic Irreversibility**. Each thermodynamic regeneration cycle permanently elevates the local basin's `entropy_floor`, guaranteeing that mathematically perfect recovery remains physically impossible over long time horizons.
