...
static int safe_strcmp(const char *s1, const char *s2, size_t max_len) {
  ...
  // If we reached max_len without a null terminator, they matched up to
  // max_len. However, if one string is supposed to be null terminated here and
  // isn't, they don't truly match. For our purposes (matching exact section
  // names), if we hit max_len without '\0', it's safest to say they do NOT
  // match if the target string (s2) expects a '\0' soon. Since s2 is usually a
  // constant like ".telos_policy", if s1 didn't hit '\0', it means s1 is
  // longer.
  return 1;
}
...
bool elf64_find_section(const uint8_t *blob, size_t size,
                        const char *section_name, uint64_t *out_offset,
                        uint64_t *out_size) {
  if (!elf64_is_valid(blob, size)) {
    return false;
  }
  ...
  // 1. Validate e_shoff (Section Header Offset) is within the binary
  // boundaries.
  if (ehdr->e_shoff == 0 ||
      ehdr->e_shoff + ((uint64_t)ehdr->e_shnum * ehdr->e_shentsize) > size) {
    return false;
  }
  ...
  const Elf64_Shdr *shstrtab_shdr =
      (const Elf64_Shdr *)(shdrs + (ehdr->e_shstrndx * ehdr->e_shentsize));
  ...
  size_t target_len = 0;
  while (section_name[target_len] != '\0')
    target_len++;

  // Iterate over all section headers
  for (uint16_t i = 0; i < ehdr->e_shnum; i++) {
    const Elf64_Shdr *shdr =
        (const Elf64_Shdr *)(shdrs + (i * ehdr->e_shentsize));
    ...
    // 3. Enforce a maximum string length boundary to prevent reading off the
    // end of memory
...
bool telos_policy_extract(const uint8_t *blob, size_t size,
                          TelosPolicyEntry *out_policy) {
  uint64_t sec_offset;
  uint64_t sec_size;

  if (!elf64_find_section(blob, size, ".telos_policy", &sec_offset,
                          &sec_size)) {
    return false;
  }
