...
static int verify_root_of_trust(const uint8_t *payload, size_t payload_size) {
  ...
  if (!elf64_find_section(payload, payload_size, ".text", &text_offset,
                          &text_size)) {
  ...
  int result = ed25519_verify(guest_signature, text_data, (size_t)text_size,
                              rot_public_key);
...
static void configure_strict_pmp(void) {
  ...
  print_hex(guest_base);
  uart_puts(" - ");
  print_hex(guest_base + guest_size);
  uart_puts("\n");
  ...
  uint64_t pmpcfg0 =
      cfg0 | (cfg1 << 8) | (cfg2 << 16) | (cfg3 << 24) | (cfg4 << 32);
...
void handle_trap(uint64_t a0) {
  ...
  if (mcause == 8) { // U-Mode ecall
    ...
    for (uint32_t i = 0; i < cap_table.count; i++) {
      ...
      if (slot->policy.intent_hash == a0 && !slot->active && !slot->revoked) {
        ...
        uart_puts(" -> Intent ");
        print_hex(a0);
        uart_puts(" GRANTED. Limit PC: ");
        print_hex_nl(slot->abs_limit_pc);
  ...
  if (mcause == 0x07 || mcause == 0x05 || mcause == 0x0F || mcause == 0x0D) {
    ...
    if (mcause == 0x0F) {
      uart_puts(" -> TCA Silicon Fault: capability was revoked at temporal "
                "boundary.\n");
...
void _start() {
  ...
  uart_puts("[VMI] Extracted ");
  print_dec(policy_count);
  uart_puts(" capability policies from Guest ELF:\n");

  /* Resolve entry point */
  uint64_t text_offset = 0;
  uint64_t text_size = 0;
  if (!elf64_find_section(guest_payload_start, payload_size, ".text",
  ...
  for (uint32_t i = 0; i < policy_count; i++) {
    ...
    cap_table.slots[i].abs_limit_pc =
        global_abs_entry + policies[i].limit_pc_offset;
    ...
    print_dec(i);
    uart_puts(": intent=");
    print_hex(policies[i].intent_hash);
    uart_puts(" limit_pc=");
    print_hex_nl(cap_table.slots[i].abs_limit_pc);
  }

  uart_puts("[VMI] Guest absolute entry point: ");
  print_hex_nl(global_abs_entry);
