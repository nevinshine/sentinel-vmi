...
struct task_offsets {
  ...
  // task_struct → active_mm (struct mm_struct *)
  uint64_t active_mm_offset;

  // mm_struct → pgd (pgd_t *)
  uint64_t mm_pgd_offset;

  // nsproxy → mnt_ns (struct mnt_namespace *)
  uint64_t nsproxy_mnt_ns_offset;
  
  // nsproxy → pid_ns_for_children (struct pid_namespace *)
  uint64_t nsproxy_pid_ns_offset;
  
  // cred → user_ns (struct user_namespace *)
  uint64_t cred_user_ns_offset;

  // Phase 6: Provenance Graph Verifier
  uint64_t pid_links_offset;
};
...
static const struct task_offsets OFFSETS_6_6 = {
    ...
    .active_mm_offset = 0x270,         // offsetof(task_struct, active_mm)
    .mm_pgd_offset = 0x50,             // offsetof(mm_struct, pgd)
    .nsproxy_mnt_ns_offset = 0x18,     // offsetof(nsproxy, mnt_ns)
    .nsproxy_pid_ns_offset = 0x20,     // offsetof(nsproxy, pid_ns_for_children)
    .cred_user_ns_offset = 0x88,       // offsetof(cred, user_ns)
};
...
static const struct task_offsets OFFSETS_6_1 = {
    ...
    .active_mm_offset = 0x268,
    .mm_pgd_offset = 0x50,
    .nsproxy_mnt_ns_offset = 0x18,
    .nsproxy_pid_ns_offset = 0x20,
