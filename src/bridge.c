...
static int open_bpf_map(void) {
  ...
  if (bpf_map_fd < 0) {
    ...
    fprintf(stderr,
            "[Bridge] Create it first: "
            "bpftool map create %s type hash "
            "key 4 value 4 entries %d name %s\n",
            VMI_ALERT_MAP_PATH, VMI_ALERT_MAP_SIZE, VMI_ALERT_MAP_NAME);
    return -1;
  }
...
static int write_alert_to_map(struct vmi_alert *alert) {
  ...
  return bpf_map_update_elem(bpf_map_fd, &alert->pid, &alert->threat_level,
                             BPF_ANY);
}
...
static int open_bpf_map(void) {
  ...
  bpf_map_fd =
      open(VMI_ALERT_FALLBACK_PATH, O_WRONLY | O_CREAT | O_APPEND, 0644);
  if (bpf_map_fd < 0) {
...
static int write_alert_to_map(struct vmi_alert *alert) {
  ...
  int n = snprintf(line, sizeof(line),
                   ...
                   alert->threat_level, alert->timestamp_ns, alert->reason);
...
static void parse_stream_config(void) {
  ...
  if (stream_enabled && strcmp(stream_mode, STREAM_MODE_HELPER) == 0 &&
    ...
    fprintf(stderr, "[Bridge] WARN: helper mode selected but "
  ...
  if (stream_enabled) {
    if (strcmp(stream_mode, STREAM_MODE_HELPER) == 0) {
      ...
      printf("[Bridge] Alert stream enabled: mode=%s target=%s:%u\n",
             stream_mode, stream_host, (unsigned int)stream_port);
    }
  }
}
...
static int stream_connect_if_needed(uint64_t now_ns) {
  ...
  if (inet_pton(AF_INET, connect_host, &addr.sin_addr) != 1) {
    close(fd);
    schedule_stream_reconnect(now_ns);
    fprintf(stderr, "[Bridge] WARN: invalid stream host '%s' (IPv4 required)\n",
            stream_host);
    return -1;
  }
...
static int stream_send_alert(const struct vmi_alert *alert) {
  ...
  if (helper_mode && !stream_helper_include_metadata) {
    ...
    len = snprintf(payload, sizeof(payload),
                   "{\"pid\":%u,\"threat_type\":\"%s\","
                   "\"confidence\":%.2f}\n",
                   alert->pid, threat_type, confidence);
  } else {
    len = snprintf(payload, sizeof(payload),
                   "{\"pid\":%u,\"threat_level\":%u,"
                   "\"threat_type\":\"%s\",\"confidence\":%.2f,"
                   "\"timestamp_ns\":%lu,\"reason\":\"%s\"}\n",
                   alert->pid, alert->threat_level, threat_type, confidence,
                   alert->timestamp_ns, reason_json);
  }
  ...
  if (helper_mode) {
    ...
    if (fputs(payload, stream_helper_fp) == EOF ||
        fflush(stream_helper_fp) != 0) {
...
static uint32_t apply_threat_policy(uint32_t pid, uint32_t requested_threat,
                                    uint64_t now_ns, int *suppress_emit,
                                    int *escalated) {
  if (suppress_emit)
    *suppress_emit = 0;
  if (escalated)
...
static void enqueue_alert(uint32_t pid, uint32_t threat_level,
                          const char *reason, int immediate) {
  ...
  strncpy(queued->alert.reason, reason ? reason : "unknown",
...
void bridge_signal_malicious(uint32_t pid, const char *reason) {
  ...
  uint32_t effective = apply_threat_policy(pid, VMI_THREAT_MALICIOUS, now_ns,
                                           &suppress, &escalated);
  if (!suppress) {
    enqueue_alert(pid, effective, reason, 1);
  }
...
void bridge_signal_suspicious(uint32_t pid, const char *reason) {
  ...
  int escalated = 0;
  uint64_t now_ns = get_timestamp_ns();
  uint32_t effective = apply_threat_policy(pid, VMI_THREAT_SUSPICIOUS, now_ns,
                                           &suppress, &escalated);
  ...
  enqueue_alert(pid, effective, reason, effective == VMI_THREAT_MALICIOUS);
