...
typedef int (*kvmi_new_guest_cb_t)(void *dom, unsigned char (*uuid)[16],
                                   void *ctx);
typedef int (*kvmi_handshake_cb_t)(
    const struct kvmi_qemu2introspector_wire *q2i,
    struct kvmi_introspector2qemu_wire *i2q, void *ctx);
...
struct kvmi_runtime {
  ...
  void *(*fn_init_unix_socket)(const char *socket,
                               ...
                               kvmi_handshake_cb_t hsk_cb, void *cb_ctx);
...
static int open_kvm_device(void) {
  ...
  if (api_ver != KVM_API_VERSION) {
    fprintf(stderr,
            "[VMI-Setup] KVM API version mismatch: expected %d, got %d\n",
...
static int parse_unix_socket_spec(const char *spec, char *path,
                                  size_t path_sz) {
  ...
  if (len == 0 || len >= path_sz ||
      len >= sizeof(((struct sockaddr_un *)0)->sun_path))
    return -1;
...
static int extract_qmp_path_from_cmdline(const char *cmdline, char *path,
                                         size_t path_sz) {
  ...
  char *saveptr = NULL;
  for (char *tok = strtok_r(tmp, " ", &saveptr); tok != NULL;
...
static int parse_kvmi_chardev_spec(const char *spec, char *path,
                                   size_t path_sz) {
  if (!spec || !path || path_sz == 0)
    return -1;
...
static int extract_kvmi_path_from_cmdline(const char *cmdline, char *path,
                                          size_t path_sz) {
  if (!cmdline || !path || path_sz == 0)
    return -1;
...
static int discover_kvmi_socket_path(const char *vm_name, int qemu_pid,
...
static int connect_qmp_channel(int qemu_pid) {
  ...
  static const char qmp_caps[] = "{\"execute\":\"qmp_capabilities\"}\n";
...
static int load_required_symbol(void *lib_handle, const char *name,
                                void **out) {
...
static int kvmi_handshake_cb(const struct kvmi_qemu2introspector_wire *q2i,
  ...
  if (q2i && q2i->name[0] != '\0' && runtime->vm_name[0] != '\0') {
    if (!strstr(q2i->name, runtime->vm_name)) {
      fprintf(stderr,
              "[VMI-Setup] WARN: KVMI handshake guest name '%s' does not match "
              "'%s'\n",
              q2i->name, runtime->vm_name);
    }
  }
...
static struct kvmi_runtime *kvmi_runtime_create(void) {
  ...
  if (load_required_symbol(runtime->lib_handle, "kvmi_init_unix_socket",
                           (void **)&runtime->fn_init_unix_socket) < 0 ||
      load_required_symbol(runtime->lib_handle, "kvmi_uninit",
                           (void **)&runtime->fn_uninit) < 0 ||
      load_required_symbol(runtime->lib_handle, "kvmi_domain_is_connected",
                           (void **)&runtime->fn_domain_is_connected) < 0 ||
      load_required_symbol(runtime->lib_handle, "kvmi_get_version",
                           (void **)&runtime->fn_get_version) < 0 ||
      load_required_symbol(runtime->lib_handle, "kvmi_memory_mapping",
                           (void **)&runtime->fn_memory_mapping) < 0 ||
      load_required_symbol(runtime->lib_handle, "kvmi_get_vcpu_count",
...
static int kvmi_runtime_connect(struct kvmi_runtime *runtime,
                                const char *socket_path, const char *vm_name) {
  ...
  runtime->listener_ctx = runtime->fn_init_unix_socket(
      socket_path, kvmi_accept_cb, kvmi_handshake_cb, runtime);
...
static int kvmi_runtime_reconnect(struct vmi_session *session) {
  ...
  if (kvmi_runtime_connect(runtime, runtime->socket_path, runtime->vm_name) <
      0) {
...
static int attach_libkvmi(struct vmi_session *session, const char *vm_name) {
  ...
  if (discover_kvmi_socket_path(vm_name, session->qemu_pid, socket_path,
  ...
  unsigned int vcpu_count = 0;
  if (runtime->fn_get_vcpu_count(runtime->dom, &vcpu_count) == 0 &&
      vcpu_count > 0 && vcpu_count <= VMI_MAX_VCPUS && session->nr_vcpus == 0) {
    session->nr_vcpus = (int)vcpu_count;
  }
...
int kvmi_session_heartbeat(struct vmi_session *session) {
  ...
  fprintf(stderr, "[VMI-Setup] KVMI heartbeat miss %u/%u\n",
          runtime->missed_heartbeats, KVMI_HEARTBEAT_MAX_MISSES);
...
static int is_candidate_ram_map(const char *perms, const char *path,
                                uint64_t span) {
  if (!perms)
    return 0;
...
static int enumerate_memslots_from_maps(struct vmi_session *session) {
  ...
  char maps_path[64];
  int n = snprintf(maps_path, sizeof(maps_path), "/proc/%d/maps",
                   session->qemu_pid);
  ...
  while (fgets(line, sizeof(line), fp) && count < VMI_MAX_MEMSLOTS) {
    ...
    int consumed = 0;
    int fields = sscanf(line, "%llx-%llx %4s %*s %*s %*s %n", &start_ull,
                        &end_ull, perms, &consumed);
    if (fields < 3)
      continue;
  ...
  for (int i = 0; i < preview; i++) {
    ...
    printf("[VMI-Setup]   slot=%u gpa=0x%lx size=0x%lx host=0x%lx\n",
           slot->slot, slot->guest_phys_addr, slot->memory_size,
...
struct vmi_session *kvmi_setup(const char *vm_name) {
  ...
  if (session->kvm_fd < 0) {
    fprintf(stderr, "[VMI-Setup] WARNING: Cannot open /dev/kvm (Bypassing for "
                    "Mock IPC testing)\n");
  ...
  if (qemu_pid >= 0) {
    ...
    if (session->vm_fd < 0) {
      fprintf(stderr,
              "[VMI-Setup] WARN: Could not duplicate kvm-vm fd from PID %d\n",
              qemu_pid);
    }
    ...
    if (session->nr_vcpus == 0) {
      fprintf(stderr,
              "[VMI-Setup] WARN: No kvm-vcpu FDs discovered from PID %d\n",
              qemu_pid);
    }
    ...
    if (session->control_fd < 0) {
      fprintf(stderr,
              "[VMI-Setup] WARN: QMP handshake unavailable for PID %d\n",
              qemu_pid);
    }

    if (attach_libkvmi(session, vm_name) < 0) {
      fprintf(stderr, "[VMI-Setup] WARN: libkvmi handshake unavailable "
                      "(running with fallback path)\n");
    }
  } else {
  ...
  if (enumerate_memslots(session) < 0) {
    fprintf(stderr, "[VMI-Setup] WARNING: Failed to discover memslots "
                    "(Bypassing for Mock IPC)\n");
    // We will continue
  }
  ...
  if (session->vcpu_fds[0] > 0) {
    ...
    if (ioctl(session->vcpu_fds[0], KVM_GET_SREGS, &sregs) == 0) {
      ...
      printf("[VMI-Setup] Captured active CR3 (init_mm.pgd) = 0x%lx (raw "
             "0x%llx)\n",
  ...
  printf("[VMI-Setup] KVM fd=%d VM fd=%d qemu_pid=%d vCPUs=%d memslots=%d "
         "control_fd=%d kvmi=%s\n",
         session->kvm_fd, session->vm_fd, session->qemu_pid, session->nr_vcpus,
         session->nr_memslots, session->control_fd,
