...
#define PT_NX (1ULL << 63) // No-execute bit
...
static struct vmi_memslot *find_memslot(struct vmi_session *s, uint64_t gpa,
                                        uint64_t *offset) {
...
static int read_remote_process(struct vmi_session *s, uint64_t remote_addr,
  ...
  while (done < size) {
    ...
    ssize_t n = process_vm_readv(s->qemu_pid, &local_iov, 1, &remote_iov, 1, 0);
    if (n < 0) {
...
static int write_remote_process(struct vmi_session *s, uint64_t remote_addr,
  ...
  while (done < size) {
    ...
    ssize_t n =
        process_vm_writev(s->qemu_pid, &local_iov, 1, &remote_iov, 1, 0);
    if (n < 0) {
      if (is_transient_errno(errno) && retries < VMI_IO_RETRY_MAX) {
        retries++;
        continue;
...
static int read_via_memslots(struct vmi_session *s, uint64_t gpa, void *buf,
                             size_t size) {
...
static int write_via_memslots(struct vmi_session *s, uint64_t gpa,
                              const void *buf, size_t size) {
  if (!s || !buf || size == 0 || !s->memslots || s->nr_memslots <= 0)
    return -1;
  ...
  while (done < size) {
    ...
    uint64_t host_addr = (uint64_t)(uintptr_t)slot->userspace_addr + offset;
    if (slot->flags & VMI_MEMSLOT_F_REMOTE_PROCESS) {
      if (write_remote_process(s, host_addr, (const char *)buf + done, chunk) <
          0) {
        return -1;
      }
    } else {
...
// ──────────────────────────────────────────────
// Phase 10: Live MMU Permission Extraction
// ──────────────────────────────────────────────
...
int vmi_mmu_translate(struct vmi_session *s, uint64_t cr3, uint64_t gva, struct page_walk_result *out) {
    ...
    uint64_t entry;
    if (vmi_read_physical(s, pml4e_addr, &entry, sizeof(entry)) < 0) return -1;
    if (!(entry & PT_PRESENT)) { out->present = false; return -1; }
    
    if (!(entry & PT_WRITABLE)) out->writable = false;
    if (!(entry & PT_USER)) out->user = false;
    if (entry & PT_NX) out->executable = false;
    
    // Level 3: PDPT
    uint64_t pdpt_base = entry & PT_ADDR_MASK;
    uint64_t pdpte_addr = pdpt_base + PDPT_INDEX(gva) * PT_ENTRY_SIZE;
    if (vmi_read_physical(s, pdpte_addr, &entry, sizeof(entry)) < 0) return -1;
    if (!(entry & PT_PRESENT)) { out->present = false; return -1; }
    ...
    if (entry & PT_PAGESIZE) {
        out->hugepage = true;
        out->gpa = (entry & 0x000FFFFFC0000000ULL) | (gva & 0x3FFFFFFF);
        return 0;
    }
    ...
    if (vmi_read_physical(s, pde_addr, &entry, sizeof(entry)) < 0) return -1;
    if (!(entry & PT_PRESENT)) { out->present = false; return -1; }
    ...
    if (entry & PT_PAGESIZE) {
        out->hugepage = true;
        out->gpa = (entry & 0x000FFFFFFFE00000ULL) | (gva & 0x1FFFFF);
        return 0;
    }
    ...
    if (vmi_read_physical(s, pte_addr, &entry, sizeof(entry)) < 0) return -1;
    if (!(entry & PT_PRESENT)) { out->present = false; return -1; }
    ...
    out->gpa = (entry & PT_ADDR_MASK) | PAGE_OFFSET(gva);
    return 0;
}
...
int vmi_gva_to_gpa(struct vmi_session *s, uint64_t cr3, uint64_t gva,
  ...
  if (vmi_mmu_translate(s, cr3, gva, &walk) < 0) return -1;
  *gpa = walk.gpa;
  return 0;
}
...
// ──────────────────────────────────────────────
// Phase 7: Canonicality Check
// ──────────────────────────────────────────────
bool mmu_is_canonical(uint64_t addr) {
    // 4-level paging (48-bit VA) canonical check:
    // Bits 48-63 must be identically 0 or 1.
    uint64_t bits_48_63 = addr >> 47;
    // For a valid canonical address, the top 17 bits must all be 0s or all be 1s.
    return (bits_48_63 == 0 || bits_48_63 == 0x1FFFF);
}
