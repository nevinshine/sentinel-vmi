...
static int hash_guest_region(struct vmi_session *s, uint64_t gpa, uint64_t size,
                             uint64_t *out_hash) {
...
static int add_env_guard_region(struct vmi_session *s, const char *name,
                                const char *gva_env, const char *size_env,
                                uint64_t default_size, int critical) {
  uint64_t gva = 0;
  ...
  if (vmi_gva_to_gpa(s, s->kernel_pgd, gva, &gpa) < 0) {
    fprintf(stderr, "[NPT-Guard] WARN: failed to resolve %s gva=0x%lx\n", name,
            gva);
    return -1;
  }

  if (add_guard_region(name, gpa, size, critical) < 0) {
    fprintf(stderr, "[NPT-Guard] WARN: failed to register guard region %s\n",
            name);
    return -1;
  }

  printf("[NPT-Guard] Optional guard '%s' enabled gva=0x%lx gpa=0x%lx "
         "size=0x%lx\n",
         name, gva, gpa, size);
...
static void register_optional_signature_regions(struct vmi_session *s) {
  (void)add_env_guard_region(s, "idt", "VMI_IDT_GVA", "VMI_IDT_SIZE",
                             DEFAULT_IDT_SIZE, 1);
  (void)add_env_guard_region(s, "gdt", "VMI_GDT_GVA", "VMI_GDT_SIZE",
                             DEFAULT_GDT_SIZE, 1);
  (void)add_env_guard_region(s, "lstar", "VMI_LSTAR_GVA", "VMI_LSTAR_SIZE",
                             DEFAULT_LSTAR_SIZE, 1);
  (void)add_env_guard_region(s, "kernel_text", "VMI_KERNEL_TEXT_GVA",
                             "VMI_KERNEL_TEXT_SIZE", DEFAULT_KERNEL_TEXT_SIZE,
                             1);
}

static int snapshot_syscall_table(struct vmi_session *s) {
  if (vmi_read_physical(s, s->syscall_table_gpa, clean_syscall_table,
  ...
  snapshot_taken = 1;
  clean_syscall_hash = fnv1a64_update(fnv1a64_init(), clean_syscall_table,
                                      sizeof(clean_syscall_table));
...
static int snapshot_guard_region(struct vmi_session *s,
  ...
  if (hash_guest_region(s, region->gpa, region->size, &hash) < 0) {
    fprintf(stderr, "[NPT-Guard] WARN: failed to baseline region '%s'\n",
            region->name);
    return -1;
  }
  ...
  printf("[NPT-Guard] Baseline region '%s' hash=0x%lx size=0x%lx\n",
         region->name, region->baseline_hash, region->size);
  return 0;
}
...
static int set_page_readonly(struct vmi_session *s, uint64_t gpa) {
  ...
  for (int i = 0; i < s->nr_memslots; i++) {
    ...
    if (gpa >= slot->guest_phys_addr &&
      ...
      region.userspace_addr = (uint64_t)slot->userspace_addr + offset;
      found_slot = 1;
      break;
    }
  }
  ...
  if (!found_slot) {
    fprintf(stderr, "[NPT-Guard] WARN: no memslot found for GPA 0x%lx\n", gpa);
    return -1;
  }
...
static int arm_guard_regions(struct vmi_session *s) {
  for (int i = 0; i < guard_region_count; i++) {
    ...
    printf("[NPT-Guard] Arming region '%s' gpa=0x%lx size=0x%lx\n",
           guard_regions[i].name, guard_regions[i].gpa, guard_regions[i].size);
...
int npt_guard_arm(struct vmi_session *s) {
  ...
  if (add_guard_region("sys_call_table", s->syscall_table_gpa,
                       SYS_CALL_TABLE_SIZE, 1) < 0) {
    return -1;
  }
...
static void report_syscall_table_diffs(struct vmi_session *s) {
  ...
  if (vmi_read_physical(s, s->syscall_table_gpa, current_table,
                        SYS_CALL_TABLE_SIZE) < 0) {
    return;
  }
  ...
  for (int i = 0; i < 512; i++) {
    if (current_table[i] != clean_syscall_table[i]) {
      ...
      npf_handler_process(s, s->syscall_table_gpa + (uint64_t)(i * 8), 1);
    }
  }
}
...
void npt_guard_handle_events(struct vmi_session *s) {
  ...
  if (last_integrity_check_us != 0 && now != 0 &&
  ...
  for (int i = 0; i < guard_region_count; i++) {
    ...
    printf("[NPT-Guard] Hash mismatch '%s': baseline=0x%lx current=0x%lx\n",
           region->name, region->baseline_hash, current_hash);

    if (strcmp(region->name, "sys_call_table") == 0) {
      report_syscall_table_diffs(s);
    } else {
      (void)npf_handler_report_integrity_violation(
          s, region->name, region->gpa, region->baseline_hash, current_hash,
          region->critical);
    }
...
int npt_guard_check_bounds(uint64_t gpa, const char **region_name,
  ...
  for (int i = 0; i < guard_region_count; i++) {
    ...
    uint64_t page_end =
        (end + VMI_PAGE_SIZE - 1) & ~(uint64_t)(VMI_PAGE_SIZE - 1);

    if (gpa >= page_start && gpa < page_end) {
      if (region_name)
        *region_name = guard_regions[i].name;
      if (is_critical)
        *is_critical = guard_regions[i].critical;
