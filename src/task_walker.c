...
static const char *const legitimate_transition_names[] = {
    "sudo", "su", "sshd", "login", "systemd", NULL,
};

static const char *const web_tier_names[] = {
    "nginx", "apache2", "httpd", "php-fpm", NULL,
};

static const char *const shell_names[] = {
    "sh", "bash", "dash", "zsh", NULL,
};
...
static int parse_btf_member_line(const char *line, char *member,
                                 size_t member_sz, uint64_t *byte_offset) {
...
static int try_load_offsets_from_btf(struct task_offsets *out) {
  ...
  FILE *fp = popen(
      "bpftool btf dump file /sys/kernel/btf/vmlinux format raw 2>/dev/null",
      "r");
  if (!fp)
    return -1;
  ...
  while (fgets(line, sizeof(line), fp) != NULL) {
    ...
    if (strstr(line, "STRUCT 'mm_struct'")) {
      in_mm = 1;
      in_task = 0;
      in_cred = 0;
      in_nsproxy = 0;
    ...
    if (in_task) {
      ...
      } else if (strcmp(member, "pid_links") == 0) {
        tmp.pid_links_offset = byte_offset;
      } else if (strcmp(member, "active_mm") == 0) {
        tmp.active_mm_offset = byte_offset;
      }
    } else if (in_cred) {
      ...
      } else if (strcmp(member, "user_ns") == 0) {
        tmp.cred_user_ns_offset = byte_offset;
      }
    } else if (in_mm) {
      if (strcmp(member, "pgd") == 0) {
        tmp.mm_pgd_offset = byte_offset;
      }
    } else if (in_nsproxy) {
      if (strcmp(member, "mnt_ns") == 0) {
        tmp.nsproxy_mnt_ns_offset = byte_offset;
      } else if (strcmp(member, "pid_ns_for_children") == 0) {
        tmp.nsproxy_pid_ns_offset = byte_offset;
      }
    }
  }
  ...
  const unsigned int required_task = TASK_SEEN_TASKS | TASK_SEEN_PID |
                                     TASK_SEEN_TGID | TASK_SEEN_COMM |
                                     TASK_SEEN_MM | TASK_SEEN_CRED;
...
static const struct task_offsets *
select_offsets_profile(const char *kernel_version) {
...
static int read_task_field(struct vmi_session *s, uint64_t task_gva,
...
static int read_tasks_next(struct vmi_session *s, uint64_t task_gva,
                           uint64_t *next_task_gva) {
  uint64_t list_next;
  if (read_task_field(s, task_gva, active_offsets->tasks_offset, &list_next,
                      sizeof(list_next)) < 0)
    return -1;
...
static int snapshot_processes(struct vmi_session *s, struct vmi_process *out,
                              int max_tasks) {
...
static struct privilege_baseline_entry *alloc_baseline(uint32_t pid) {
  ...
  for (int i = 1; i < MAX_PRIV_BASELINE; i++) {
    if (privilege_baseline[i].generation <
        privilege_baseline[oldest].generation)
      oldest = i;
  }
...
int task_walker_read_process(struct vmi_session *s, uint64_t task_gva,
  ...
  // PID
  if (read_task_field(s, task_gva, active_offsets->pid_offset, &out->pid,
                      sizeof(out->pid)) < 0)
    return -1;

  // TGID
  if (read_task_field(s, task_gva, active_offsets->tgid_offset, &out->tgid,
                      sizeof(out->tgid)) < 0)
    return -1;
  ...
  if (active_offsets->real_parent_offset != 0) {
    ...
    if (read_task_field(s, task_gva, active_offsets->real_parent_offset,
      ...
      vmi_read_virtual(s, s->kernel_pgd,
                       parent_task + active_offsets->pid_offset, &out->ppid,
                       sizeof(out->ppid));
    }
  }

  // comm (process name)
  if (read_task_field(s, task_gva, active_offsets->comm_offset, out->comm,
                      TASK_COMM_LEN) < 0)
    return -1;
  out->comm[TASK_COMM_LEN - 1] = '\0';

  // mm_struct pointer
  if (read_task_field(s, task_gva, active_offsets->mm_offset, &out->mm_addr,
                      sizeof(out->mm_addr)) < 0)
    return -1;
  ...
  // cred pointer
  if (read_task_field(s, task_gva, active_offsets->cred_offset, &out->cred_addr,
                      sizeof(out->cred_addr)) < 0)
    return -1;
  ...
  if (out->cred_addr) {
    ...
    if (active_offsets->cred_cap_effective_offset != 0) {
      (void)vmi_read_virtual(s, s->kernel_pgd,
                             out->cred_addr +
                                 active_offsets->cred_cap_effective_offset,
                             &out->cap_effective, sizeof(out->cap_effective));
    }
  }
...
void task_walker_dump(struct vmi_session *s) {
  ...
  printf("[TaskWalker] %-6s %-6s %-6s %-16s %-5s %-5s %-10s %-18s\n", "PID",
         "TGID", "PPID", "COMM", "UID", "EUID", "CAP_EFF", "TASK_ADDR");
  printf("[TaskWalker] ─────────────────────────────────"
         "──────────────────────\n");
  ...
  do {
    ...
    printf("[TaskWalker] %-6u %-6u %-6u %-16s %-5u %-5u 0x%08lx 0x%lx\n",
           proc.pid, proc.tgid, proc.ppid, proc.comm, proc.uid, proc.euid,
           proc.cap_effective, proc.task_addr);
...
int task_walker_find_pid(struct vmi_session *s, uint32_t pid,
                         uint64_t *task_addr) {
  if (!s || !task_addr || s->init_task_addr == 0)
    return -1;
  ...
  do {
    uint32_t this_pid;
    if (read_task_field(s, current, active_offsets->pid_offset, &this_pid,
                        sizeof(this_pid)) < 0)
      return -1;
  ...
  for (int i = 0; i < nr; i++) {
    ...
    if (had_baseline) {
      ...
      if (root_transition && !is_legitimate_priv_transition(proc)) {
        char reason[96];
        snprintf(reason, sizeof(reason), "priv_escalation pid=%u comm=%s",
                 proc->pid, proc->comm);
      ...
      if (caps_expanded && !is_legitimate_priv_transition(proc)) {
        char reason[96];
        snprintf(reason, sizeof(reason), "cap_expansion pid=%u comm=%s",
                 proc->pid, proc->comm);
        printf("[TaskWalker] ⚠ CAP EXPANSION: PID %u (%s) 0x%lx->0x%lx\n",
               proc->pid, proc->comm, entry->cap_effective,
               proc->cap_effective);
        bridge_signal_suspicious(proc->pid, reason);
        detections++;
      }
...
int task_walker_detect_orphans(struct vmi_session *s) {
  ...
  for (int i = 0; i < nr; i++) {
    ...
    if (!pid_exists(procs, nr, procs[i].ppid)) {
      char reason[96];
      snprintf(reason, sizeof(reason), "orphan_process pid=%u ppid=%u",
...
int task_walker_detect_fork_bomb(struct vmi_session *s, uint32_t threshold) {
  ...
  for (int i = 0; i < nr; i++) {
    ...
    if (children >= threshold) {
      char reason[96];
      snprintf(reason, sizeof(reason), "fork_bomb pid=%u children=%u",
               parent_pid, children);
      printf(
          "[TaskWalker] ⚠ FORK BOMB PATTERN: parent PID %u has %u children\n",
          parent_pid, children);
...
int task_walker_detect_suspicious_ancestry(struct vmi_session *s) {
  ...
  for (int i = 0; i < nr; i++) {
    ...
    if (string_in_list(parent->comm, web_tier_names)) {
      char reason[96];
      snprintf(reason, sizeof(reason), "suspicious_ancestry pid=%u parent=%u",
               procs[i].pid, parent->pid);
      printf(
          "[TaskWalker] ⚠ SUSPICIOUS ANCESTRY: %s(%u) spawned shell %s(%u)\n",
          parent->comm, parent->pid, procs[i].comm, procs[i].pid);
      bridge_signal_suspicious(procs[i].pid, reason);
      detections++;
    }
  }
