...
static void remove_fallback_log(void) { unlink(VMI_ALERT_FALLBACK_PATH); }
...
static void test_alert_constants(void) {
  ...
  if (VMI_THREAT_CLEAN == 0 && VMI_THREAT_SUSPICIOUS == 1 &&
      VMI_THREAT_MALICIOUS == 2 && VMI_ALERT_MAP_SIZE > 0) {
    PASS();
  } else {
...
static void test_stream_helper_payload_schema(void) {
  ...
  setenv("VMI_ALERT_STREAM_ENABLE", "1", 1);
  setenv("VMI_ALERT_STREAM_MODE", "helper", 1);
  setenv("VMI_ALERT_GRPC_HELPER_CMD",
         "cat >>/tmp/vmi_helper_stream_payload.log", 1);
  ...
  int has_type =
      file_contains_text(helper_out, "\"threat_type\":\"malicious\"");
...
static void test_stream_helper_auto_enable(void) {
  ...
  setenv("VMI_ALERT_STREAM_ENABLE", "0", 1);
  setenv("VMI_ALERT_STREAM_MODE", "helper", 1);
  setenv("VMI_ALERT_GRPC_HELPER_CMD", "cat >>/tmp/vmi_helper_auto_enable.log",
         1);
